...
########## Coding by Yang 2020.10 ######## modify loss function #####################################
class ModifyLoss(torch.nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self, mask_pred, mask_contour, mask):
        n,_,w,h = mask.shape  # n: batch size, w: width, h: high 
        belta = 1             # emphasis factor of edge 
        num_all = n*w*h       # number of all pixels
        num_pos = torch.sum(mask[:,0,:,:]) # number of foreground pixels
        alpha = num_pos/num_all            # balance factor between foreground and background pixels
        mask_pred = 1/(1+torch.exp(-mask_pred/2)) # sigmoid function, /2 enlarge the gap
        # foreground loss and background loss
        loss1 = (1-alpha)*(mask[:,0,:,:] + belta*mask_contour[:,0,:,:])*torch.log(mask_pred[:,0,:,:]+1e-10) + alpha*(1-mask[:,0,:,:])*torch.log(1-mask_pred[:,0,:,:]+1e-10)
