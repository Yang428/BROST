...
def save_debug(data, pred_mask):
    ...
    test_img = data['train_images'][:, batch_element, :, :].permute(1, 2, 0)
    test_mask = data['train_masks'][0, batch_element, :, :]
...
class SegmActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        masks_pred = self.net(data['train_images'].permute(1, 0, 2, 3),
                              data['train_images'].permute(1, 0, 2, 3),
                              data['train_init_masks'].permute(1, 0, 2, 3),
                              test_dist)  

        masks_gt = data['train_masks'].permute(1, 0, 2, 3) 
