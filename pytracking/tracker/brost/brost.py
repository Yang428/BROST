...
class BROST(BaseTracker):
    ...
    ## update background features for segmentation ##
    def update_bkgd_feat_segm(self, image, pos, sz):
        ...
        segm_pred = self.segm_net.segm_predictor(test_feat_segm, self.train_feat_segm, train_masks, test_dist_map, self.train_feat_bkgds, self.bkgd_masks, segm_update_flag=self.segm_update_flag)
        ...
        if len(cnt_area) > 0 and len(contours) != 0 and np.max(cnt_area) > 50:
            ...
            mask_gpu = torch.unsqueeze(torch.unsqueeze(torch.tensor(1-mask).type(torch.float32), dim=0), dim=0).to(self.params.device)

            if len(self.bkgd_masks) == 4:   ## bag size is 4, if the number of backgrounds is exceeded, pop the earliest one
                self.train_feat_bkgds.pop(0)
                self.bkgd_masks.pop(0)        
            self.train_feat_bkgds.append(test_feat_segm) ## append new backgound features
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        sample_sz = self.pre_target_sz.clone()
        ...
        if thr > self.params.tracking_confidence_thr or self.frame_num < (self.params.score_budget_sz+1):
        ...
        if uncert_score < self.params.tracking_uncertainty_thr:
            if thr > self.params.filter_sample_thr: ## filter the training samples
                test_x = self.extract_processed_sample(im, new_pos, sample_scales, self.img_sample_sz)
                train_x = TensorList([x[scale_ind:scale_ind + 1, ...] for x in test_x])
                train_y = self.get_label_function(new_pos, sample_scales[scale_ind])
                self.update_memory(train_x, train_y, learning_rate)
            ## update segmentation background features ##
            if hard_negative or self.segm_hard_neg:  ## if there is hard negative in background, update 10 skip
                if (self.frame_num-self.segm_update_num) > (self.params.segm_update_skip-1):  
                    self.update_bkgd_feat_segm(image, new_pos, self.target_sz)
                    self.segm_update_num = self.frame_num
                    self.segm_update_flag = True  
            elif (self.frame_num-self.segm_update_num) > (self.params.segm_update_skip+9): ## update 20 skip
                self.update_bkgd_feat_segm(image, new_pos, self.target_sz)
                self.segm_update_num = self.frame_num
                self.segm_update_flag = True    
            elif self.frame_num < (self.params.segm_update_skip+1): ## update the first 10 frames
                self.update_bkgd_feat_segm(image, new_pos, self.target_sz)
                self.segm_update_num = self.frame_num
                self.segm_update_flag = True            
        ...
        if (not need_refine_flag):
            output_state = pred_segm_region
        else:
    ...
    def init_segmentation(self, image, bb, init_mask=None):
        ...
        ## load mask init network
        segm_net_maskInit, _ = load_network(self.params.segm_maskInitNet_path, backbone_pretrained=False,
                                   constructor_module='ltr.models.segm.segm',
                                   constructor_fun_name='segm_resnet50')

        if self.params.use_gpu:
            segm_net.cuda()
            segm_net_maskInit.cuda()
        segm_net.eval()
        segm_net_maskInit.eval()

        for p in segm_net.segm_predictor.parameters():
            p.requires_grad = False
        for p in segm_net_maskInit.segm_predictor.parameters():
            p.requires_grad = False
        ...
        if init_mask is None:
            ...
            while iters < 1:
                # Obtain segmentation prediction
                ## mask init
                segm_pred = segm_net_maskInit.segm_predictor(test_feat_segm, train_feat_segm, train_masks, test_dist_map)
        ...
        self.segm_net = segm_net               ## segmentation network
        self.train_feat_segm = train_feat_segm ## segmentation features of the first frame
        self.init_mask_patch = mask_gpu        ## initial mask of the first frame
        self.train_feat_bkgds = []             ## init the bag of background features
        self.bkgd_masks = []                   ## mask for background 
        self.segm_hard_neg = False             ## flag of hardnegative 
    ...
    def segment_target(self, image, pos, sz):
        ...
        if len(cnt_area) > 0 and len(contours) != 0 and np.max(cnt_area) > 20: 
            ## found negative
