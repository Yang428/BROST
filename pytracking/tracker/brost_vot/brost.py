...
class BROST(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        # Compute output bounding box for VOT
        aa_box = torch.cat((self.pos[[1,0]] - (self.target_sz[[1,0]]-1)/2, self.pos[[1,0]] + (self.target_sz[[1,0]]-1)/2)).tolist()
        ...
        if need_refine_flag:
            ...
            if hasattr(self, 'pos_iounet'):
                aa_box = torch.cat((self.pos_iounet[[1,0]] - (self.target_sz_iounet[[1,0]]-1)/2, self.pos_iounet[[1,0]] + (self.target_sz_iounet[[1,0]]-1)/2)).tolist()        
        new_state = [aa_box[0], aa_box[1], aa_box[0], aa_box[3], aa_box[2], aa_box[3], aa_box[2], aa_box[1]] 
        if (not need_refine_flag):
            output_state = pred_region_rotate
        else:
